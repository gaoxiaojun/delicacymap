...
void MapDataSource::GetRelatedUsers( int uid, UserRelation relation, ProtocolBuffer::UserList *users, google::protobuf::Closure *done )
{
    // User relation is specified in DBManager/RelationConstants.cs, as follows:
    query.Clear();
    query.set_uid(uid);
    query.set_relation((int)relation);
    GetRelatedUsers(&query, users, done);
}

void MapDataSource::SetUserRelation( int uid, int target_uid, UserRelation relation, google::protobuf::Closure *done )
{
    query.Clear();
    query.set_uid(uid);
    query.set_uid_target(target_uid);
    query.set_relation((int)relation);
    SetUserRelation(&query, done);
}

void MapDataSource::SetUserRelation( ProtocolBuffer::Query *query, google::protobuf::Closure *done )
{
...
void MapDataSource::AddRestaurant(ProtocolBuffer::Query *query, ProtocolBuffer::Restaurant *r, google::protobuf::Closure *done)
{
...
void MapDataSource::RegisterUser( const std::string& username, const std::string& md5PWD, const std::string& nickname, ProtocolBuffer::User* result, google::protobuf::Closure* done )
{
    query.Clear();
    query.set_emailaddress(username);
    query.set_password(md5PWD);
    query.set_nickname(nickname);
